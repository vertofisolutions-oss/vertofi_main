...
export const api = {
  ...
  // ── Professional (CA/CMA/CS/…) self-registration + KYC ──
  registerProfessional: (body: Record<string, unknown>) =>
    request<{ userId: string; publicId: string | null; tokens: { accessToken: string; refreshToken: string } }>(
      "/auth/register-professional",
      { method: "POST", body: JSON.stringify(body) },
      false,
    ),
  requiredDocs: (type: string) =>
    request<{ type: string; label: string; required: boolean }[]>(`/auth/professional/required-docs/${type}`, {}, false),
  myProfessionalProfile: () => request<Record<string, any> | null>("/auth/professional/profile"),
  updateProfessionalProfile: (body: Record<string, unknown>) =>
    request<{ updated: boolean }>("/auth/professional/profile", { method: "PUT", body: JSON.stringify(body) }),
  kycPresign: (type: string, filename: string, contentType: string) =>
    request<{ documentId: string; uploadUrl: string; key: string }>(
      "/documents/kyc/presign",
      { method: "POST", body: JSON.stringify({ type, filename, contentType }) },
    ),
  kycCommit: (documentId: string) =>
    request<{ documentId: string; type: string; filename: string; status: string }>(
      `/documents/kyc/${documentId}/commit`,
      { method: "POST" },
    ),
  kycList: (userId: string) => request<Record<string, any>[]>(`/documents/kyc/list/${userId}`),

  // admin: professional verification queue
  pendingProfessionals: () => request<Record<string, any>[]>("/auth/professional/pending"),
